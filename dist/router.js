        ...
        return function (context, next) {
            return __awaiter(this, void 0, Promise, function* () {
                ...
                if (matchedRoute) {
                    ...
                    let body = {};
                    //parse body
                    if (matchedRoute.consume === util_1.MediaType.JSON && context.headers['content-type'] === util_1.mediaTypeToString(util_1.MediaType.JSON)) {
                        body = yield new Promise((resolve, reject) => {
                            parse.json(context).then(resolve).catch(reject);
                        });
                    } else if (matchedRoute.consume === util_1.MediaType.FORM && context.headers['content-type'] === util_1.mediaTypeToString(util_1.MediaType.FORM)) {
                        body = yield new Promise((resolve, reject) => {
                            parse.form(context).then(resolve).catch(reject);
                        });
                    } else if (matchedRoute.consume === util_1.MediaType.TEXT && context.headers['content-type'] === util_1.mediaTypeToString(util_1.MediaType.TEXT)) {
                        body = yield new Promise((resolve, reject) => {
                            parse.text(context).then(resolve).catch(reject);
                        });
                    } else if (matchedRoute.consume === util_1.MediaType.MULTIPART && (context.headers['content-type'] || '').substr(0, 19) === util_1.mediaTypeToString(util_1.MediaType.MULTIPART)) {
